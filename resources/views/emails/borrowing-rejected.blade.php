<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Peminjaman Ditolak</title></head>
...
<div style="max-width:600px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
  <div style="background:linear-gradient(135deg,#991b1b,#dc2626);padding:32px 24px;text-align:center;">
  ...
  <div style="padding:32px 28px;">
    <h2 style="margin:0 0 8px;font-size:20px;color:#991b1b;">❌ Peminjaman Buku Ditolak</h2>
    <p style="margin:0 0 20px;color:#64748b;font-size:14px;">{{ now()->format('d M Y, H:i') }} WIB</p>
    <p>Halo <strong>{{ $user->name }}</strong>,</p>
    <p>Mohon maaf, pengajuan peminjaman buku Anda <strong>tidak dapat disetujui</strong> pada saat ini.</p>
    @php $book = $borrowing->details->first()?->book; @endphp
    <div style="background:#fef2f2;border-left:4px solid #ef4444;border-radius:8px;padding:18px 20px;margin:20px 0;">
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ...
        <tr><td style="color:#64748b;padding:4px 0;">Status</td><td><span style="background:#fee2e2;color:#991b1b;padding:2px 10px;border-radius:20px;font-size:12px;font-weight:600;">Ditolak</span></td></tr>
        @if($reason)
        <tr><td style="color:#64748b;padding:4px 0;vertical-align:top;">Alasan</td><td>{{ $reason }}</td></tr>
        @endif
      </table>
    </div>
    <p>Anda dapat mencoba mengajukan peminjaman kembali setelah kondisi memungkinkan, atau menghubungi petugas perpustakaan untuk informasi lebih lanjut.</p>
    <div style="text-align:center;margin:28px 0;">
      <a href="{{ config('app.url') }}/user/catalog" style="background:#1d4ed8;color:#fff;text-decoration:none;padding:12px 28px;border-radius:8px;font-weight:600;font-size:14px;display:inline-block;">Jelajahi Katalog Buku</a>
    </div>
    <p style="margin-bottom:0;">Terima kasih atas pengertian Anda,<br><strong>Tim Perpustakaan Tiga Serangkai</strong></p>
  </div>
