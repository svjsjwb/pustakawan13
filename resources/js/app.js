...
function activeReservationCountForMember(memberId) {
...
function memberHasActiveActivity(memberId) {
...
function syncMemberStatus(memberId) {
  ...
  member.status = memberHasActiveActivity(memberId)
    ? 'aktif'
    : 'nonaktif';
}

function syncAllMemberStatuses() {
  members.forEach(member => {
    syncMemberStatus(member.id);
  });
}
...
function renderDashboard() {
  ...
  const acts = loans.slice().sort((a, b) => (b.returnDate || b.loanDate) > (a.returnDate || a.loanDate) ? 1 : -1).slice(0, 6).map(l => {
    ...
    return {
      icon: returned ? '↩' : '📖', color: returned ? 'var(--success)' : 'var(--primary)', bg: returned ? 'var(--success-soft)' : 'var(--primary-soft)',
...
function populateLoanForm() {
  document.getElementById('loan-member').innerHTML =
    members.map(m => `<option value="${m.id}">${m.name} (${m.no})</option>`).join('');
  document.getElementById('loan-book').innerHTML =
    books.filter(b => b.available > 0)
      .map(b => `<option value="${b.id}">${b.title} — sisa ${b.available}</option>`)
      .join('') ||
  ...
  document.getElementById('res-member').innerHTML =
    members.map(m => `<option value="${m.id}">${m.name} (${m.no})</option>`).join('');
  document.getElementById('res-book').innerHTML =
    books.map(b => `<option value="${b.id}">${b.title} ${b.available <= 0 ? '(dipinjam)' : '(tersedia)'}</option>`).join('');
}
function createLoan() {
...
function returnLoan(id) {
  ...
  syncMemberStatus(l.memberId);
  const fine = calcFine(l);
  toast(
    fine > 0
      ? `Buku dikembalikan. Denda keterlambatan: ${rupiah(fine)}`
...
function renderMembers() {
  ...
  document.getElementById('members-table-body').innerHTML = list.map(m => {
    ...
    const statusBadge = m.status === 'aktif' ? '<span class="status-badge aktif">Aktif</span>' : '<span class="status-badge nonaktif">Tidak Aktif</span>';
    return `<tr>
